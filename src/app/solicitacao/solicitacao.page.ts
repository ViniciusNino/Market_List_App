...
export class SolicitacaoPage implements OnInit {
  ...
  ngOnInit() {
    this.usuarioLogado = JSON.parse(
      localStorage.getItem(USUARIO.USUARIOAUTENTICAR)
    );
    this.listar_itens();
  ...
  async listar_itens() {
    this.itens = await this.solicitacaoApi.getItemPorUnidade(
      this.usuarioLogado.unidadeId
    );
  }
  ...
  cancelar() {
    this.router
      .navigateByUrl(ROUTES_COMPONENTS.HOME_SOLICITANTE)
      .then((nav) => {
